...
<?= $block->end() ?>
    ...
    <div data-role="navbar">
        <ul class="nav-tab">
            <li><a href="<?= $wei->url('coupon/index', array('used' => 'no', 'v'=>rand(1, 100000))) ?>" <?php if($used == 'no'):?>class="ui-btn-active"<?php endif;?>>未使用优惠券</a></li>
            <li><a href="<?= $wei->url('coupon/index', array('used' => 'yes', 'v'=>rand(1, 100000))) ?>" <?php if($used == 'yes'):?>class="ui-btn-active"<?php endif;?>>已用优惠券</a></li>
        </ul>
    </div><!-- /navbar -->
                ...
                <li data-icon="false">
                	<a href="#">
                        ...
                        <p>
                            ...
                            <span class="coupon-list">有效期: <?= substr($value['startTime'], 0, 10)?>-- <?= substr($value['endTime'], 0, 10)?><?php if(($value['endTime'] < date('Y-m-d H:i:s')) && ($used == 'no')){echo '<span style="color:red;">(已过期)</span>';}?></span>
                        	<?php if($used == 'no'):?>
